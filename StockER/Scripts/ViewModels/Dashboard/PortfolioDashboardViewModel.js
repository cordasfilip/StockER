function PortfolioDashboardViewModel(id)
{
    ...
    self.GoToInfo = function ()
    {
        ...
        var linq = "http://www.bing.com/news/search?q=" + data.Date + "+" + data.Stock + "&qs=n&form=QBNT&pq=" + data.Date + "+" + data.Stock + "&sc=2-11&sp=-1&sk=";
    ...
    $(App.SystemOperations.NMF).on("Started", CalculationStart);
    $(App.SystemOperations.NMF).on("Ended", CalculationDone);
    db.GetAsync(id).done(UpdatePortfolio);
    
    function UpdateEvent(e, update)
    {
        var port = Enumerable.From(update).FirstOrDefault(null, "r=>r.Item.Id==" + self.Portfolio.Id);
        if (port!=null)
        {
            UpdatePortfolio(port.Item, port.Props);
        }       
    }

    function PortfolioRemoved(e,portfolios)
    {
        if (Enumerable.From(portfolios).Any("r=>r.Id==" + self.Portfolio.Id))
        {
            location = "#/";
            Utils.ShowInfo("Portfolio removed going to Home page ");
        }
    }
    ...
    function CalculationStart(e, nid, date)
    {
        if (self.Portfolio.Id == nid)
        {
            Utils.ShowInfo("Calculation started for portfolio " + nid + "Time" + kendo.toString(new Date(), "hh-mm-ss"));
            self.Loading(true);
        }
    }

    function CalculationDone(e, nid, date)
    {
        if (self.Portfolio.Id == nid)
        {
            Utils.ShowInfo("Calculation ended for portfolio " + nid+ "Time" + kendo.toString(new Date(),"hh-mm-ss"));
            self.FactorsVM(new FactorsViewModel(self));
        }
    }
    ...
    self.Dispose = function ()
    {
        ...
        $(App.SystemOperations.NMF).off("Started", CalculationStart);
        $(App.SystemOperations.NMF).off("Ended", CalculationDone);
    }

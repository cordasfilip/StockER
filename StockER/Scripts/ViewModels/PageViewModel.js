function PageViewModel()
{
    ...
    var globalActions =
        {
            NewPortfolio: function ()
            {
                ...
                {
                    App.DataAccess.PortfolioDB.AddPortfolio(portfolio).done(function (port)
                    {
                        location = "#/Dashboard/Portfolio/" + + port.Id;
                    });
                });
    ...
    $(App.DataAccess.PortfolioDB).on("Added", function (e,portfolios)
    {
        ...
        Utils.ShowInfo("Portfolio Added");
       
    });

    $(App.DataAccess.PortfolioDB).on("Removed", function (e, portfolios)
    {
        var portfolio;
        for (var i = 0; i < portfolios.length; i++)
        {
            portfolio = portfolios[i];
            var value = leftNavigationItems.First("r=>r.Id=='Portfolios'").Value;
            var port = value.Items;
            value.Items = Enumerable.From(port).Where("r=>r.Id!=" + portfolio.Id).ToArray();

            leftNavigationItems = App.DataAccess.MetaDataDB.GetLeftNavigationItems(value.Items);

            self.LeftNavigationItems([]);
            self.LeftNavigationItems(leftNavigationItems.ToArray());
        }
        Utils.ShowInfo("Portfolio Removed");
